...
async function ServiceContent({ slug }: { slug: string }) {
  ...
  // Use packages_include from Airtable if available, otherwise use default features
  const features = service.packages_include 
    ? service.packages_include.split(',').map(item => item.trim()) 
    : [
  ...
  // Use redirectLink if available, otherwise use gumroadLink and ensure it has the right parameters
  let purchaseLink = service.redirectLink || service.gumroadLink;
  
  // Add redirect parameter to Gumroad link if it doesn't have one
  if (purchaseLink.includes('gumroad.com') && !purchaseLink.includes('wanted=true')) {
  ...
  return (
    <>
        <section className="relative py-16 overflow-hidden">
          ...
          {service.tagline && (
            <p className="text-2xl text-purple-600 mb-4">{service.tagline}</p>
          )}
          <p className="text-xl text-gray-600 mb-8 max-w-3xl">{service.description.substring(0, 120)}{service.description.length > 120 ? '...' : ''}</p>
            ...
            <div className="flex flex-wrap gap-4 mb-8">
              ...
              {service.number_of_purchases && service.number_of_purchases > 0 && (
                <div className="bg-blue-100 px-4 py-2 rounded-full text-blue-800 font-medium text-sm flex items-center">
                  {service.number_of_purchases}+ customers
              </div>
              )}
            </div>
          </div>
